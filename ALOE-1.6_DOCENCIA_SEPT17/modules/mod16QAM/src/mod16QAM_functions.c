 ...
 * @param S_out pointer to complex symbol */
/*inline void modulate_16QAM(char *bits, _Complex float *S_out)
{
	int k;
...
{
	int k, n, j;
//	printf("modulate_16QAM()\n");
	for(n=0; n<num_bytes; n++){
//		printf("%x,", ((int)*(bytes+n))&0xFF);
		j=2*n;
		k = (*(bytes+n))&0x0F;
		*(S_out+j) = qam16_table[k];
		k = ((*(bytes+n))>>4)&0x0F;
		*(S_out+j+1) = qam16_table[k];	
	}
	return(2*num_bytes);
}
