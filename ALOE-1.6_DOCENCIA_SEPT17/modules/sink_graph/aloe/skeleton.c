...
void ConfigInterfaces() {
	...
	if (param_init(parameters,nof_parameters) == -1) {
		printf("Error configuring sink_plot_parameters.h\n");
		return;
	}
	...
	for (i = 1; i < nof_input_itf+1; i++) {
		...
		input_itfs[i].buffer = &input_buffer[(i-1)*input_max_samples];

//		printf("sink_graph/aloe/skeleton: input_itfs[%d].buffer=%u\n", i, (unsigned int)input_itfs[i].buffer);
	...
	for (i = 0; i < nof_output_itf; i++) {
		...
		output_itfs[i].buffer = &output_buffer[i*output_max_samples];

		printf("sink_graph/aloe/skeleton: output_itfs[%d].buffer=%u\n", i, (unsigned int)output_itfs[i].buffer);
