...
//	printf("FFT_IFFT: rcv_samples=%d, opmode=%d\n", rcv_samples, op_mode);

	if (rcv_samples == 0)return(0);
	// Check if data exceed maximum expected data
/*	if (rcv_samples != block_length) {
		printf("fft_ifft.work(): ERROR rcv_samples = %d do not match expected input block_length=%d\n", rcv_samples, block_length);
		printf("fft_ifft.work(): REMIND block_length=fftsize*numffts: block_length=%d, fftsize=%d, numffts=%d\n", block_length, fftsize, numffts);
		return(-1);
	}
*/
	...
	/* do DSP stuff here */
	//Copy received data to FFT buffer
	for(i=0; i<numffts; i++){
		k=i*fftsize;
		for(n=0; n<fftsize; n++)*(fftin+n)=(fftw_complex) *(input+k+n);
		//Execute FFT
		fftw_execute(fftplan);
		//Copy processed data to output buffer
		if(op_mode==1)for(n=0; n<fftsize; n++)*(output+k+n)=(_Complex float) *(fftout+n);
		if(op_mode==2)for(n=0; n<fftsize; n++)*(output+k+n)=((_Complex float) *(fftout+n))/fftsize;
	}
	snd_samples=rcv_samples;
	// Indicate the number of samples at autput 0 with return value
	//printf("Tslot=%d || %s: rcv_samples=%d, snd_samples=%d\n", Tslot, mname, rcv_samples, snd_samples);
