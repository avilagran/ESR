...
int initialize() {
	...
	printf("\033[1;37;46m\tO--------------------------------------------------------------------------------------------O\t\033[0m\n");
	printf("\033[1;37;46m\tO    SPECIFIC PARAMETERS SETUP: %s                                   	\t\033[0m\n", mname);
	printf("\033[1;37;46m\tO      Nof Inputs=%d, DataTypeIN=%s, Nof Outputs=%d, DataTypeOUT=%s                    \t\033[0m\n", 
		       NOF_INPUT_ITF, IN_TYPE, NOF_OUTPUT_ITF, OUT_TYPE);
	printf("\033[1;37;46m\tO      block_length=%d, ctrl_flow=%d, snd_flag=%d                                             \t\033[0m\n", block_length, ctrl_flow, snd_flag);
	printf("\033[1;37;46m\tO      mode=%d [0: Send File, 1: Sent Data Blocks]                                            \t\033[0m\n", mode);
	printf("\033[1;37;46m\tO      debug=%d                                                                               \t\033[0m\n", debug);
	printf("\033[1;37;46m\tO--------------------------------------------------------------------------------------------O\t\033[0m\n");
...
/*	if(mode==0){
		...
		strcat(aux, IDchar);
		strcat(BERfile, aux);
		BERp=datafile_open(BERfile, "w");
		writemy(BERp, IDcapt, 16);
	...
	if(mode==0){
		...
		if(rcv_samples>0){
			if(compare_data(filebuffer, myfilelength, inp, rcv_samples, &BER)==-1){
				datafile_write_float(BERp,  &BER, 1);
				return(-1);
			}
		}
	}
