...
float Afreq=50.0; 		// Scanning resolution in Hz
int delay_value=0;
int modephase=0; 		// 0: phase as output; 1: group delay as output
...
int initialize() {
	...
	param_get_float("modephase", &modephase);
	...
	printf("O      opmode=%d [0: Impulse, 1: Frequency Scanning], frequency_step=%3.1f Hz\n", opmode, Afreq);
	printf("O      pulselength=%d, gain=%3.3f, threshold=%3.5f\n", pulselength, gain, threshold);
	printf("O      modephase=%d [0: Phase in rad, 1: Group Delay in sec]\n", modephase);
	printf("O--------------------------------------------------------------------------------------------O\n");
	...
	if(opmode==1){
		...
		if(fase==1){
			...
			if(Tslot <= (scaningTslots+5)*NOFTS){
				...
				// Sent cosinus: Only coswot
				for(i=0; i<block_length; i++){
					__real__ output0[i] =__real__ sentcos[i];
					__imag__ output0[i] = 0.0;
				}
				...
				if(LTslot == NOFTS-1){
					...
					// Compute phase
					CHphase = get_CH_phase_radA(Csentcos, myin, block_length, aux);
				...
				for(i=0; i<spectrum_length; i++){
					...
					__imag__ output1[i]=CHPHASE[i]; //Modificat
				}
				// Indicate number of samples output 1
				set_output_samples(1,  spectrum_length);

			}else{
				// CORRECT PHASE: Positive Slope and between -PI and PI			
				if(END==0){
				// Calculate Group Delay
					correct_phase(CHPHASE, spectrum_length, phase2COUT, modephase, Afreq);//modificat
					if(modephase == 1){
						computeGroupDelay(CHPHASE, spectrum_length, phase2COUT, Afreq);
					}
					END=1;
				}
				for(i=0; i<spectrum_length; i++){
