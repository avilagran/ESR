...
#include "DCP_interfaces.h"
#include "DCP_functions.h"
#include "DCP.h"

//ALOE Module Defined Parameters. Do not delete.
char mname[STR_LEN]="removeCP";
int run_times=1;
...
int initialize() {
	...
	if (block_length > get_input_max_samples()) {
		...
		moderror("Check your removeCP_interfaces.h file\n");
		return -1;
	}
	/*Include the file name and line number when printing*/
...
int work(input_t *inp, output_t *out) {
	...
	for(i=0;i < 14;i++){
		...
		if(i==7) offset--;
		if(i==8) offset=2;
		memcpy(out+i*128,inp+(extendedCP*10)+(normalCP*9)+(i*128)+CPlen+offset,sizeof(_Complex float)*128);
	}
	int snd_samples=1792;
	return snd_samples;
}
